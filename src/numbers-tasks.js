...
function isPrime(n) {
  ...
  if (n > 7) {
    const sqrn = Math.ceil(Math.sqrt(n));
    for (let i = 11; i <= sqrn; i += 1)
      if ((i - 1) % 6 === 0 || (i + 1) % 6 === 0) if (n % i === 0) return false;
  }
  return true;
}
...
function toNumber(value, def) {
  if (typeof value === 'undefined' || typeof value === 'object') return def;
...
function getRandomInteger(min, max) {
  return min + Math.round((max - min) * Math.random(min, max));
}
...
function getCountOfOddNumbers(number) {
  let sum = 0;
  for (let i = 0; i <= Math.abs(number); i += 1) if (i % 2 !== 0) sum += 1;
  return sum;
}
